...
window.onload = function() {
  space.image.src =
    "https://res.cloudinary.com/dv3lue3qa/image/upload/v1554025845/space-bg-01_-_Copy.png";
...
// Blue Asteroid
function rotateBlueAsteroid(image, x, y, width, height) {
  let time = new Date();
  ctx.save();
  ctx.rotate(
    ((2 * Math.PI) / 9) * time.getSeconds() +
    ((2 * Math.PI) / 1000) * time.getMilliseconds()
  );
  ctx.translate(0, 120.5);
  ctx.drawImage(blueAsteroid.image, -0.5, -3.5, 100, 70);
  ctx.restore();
}
